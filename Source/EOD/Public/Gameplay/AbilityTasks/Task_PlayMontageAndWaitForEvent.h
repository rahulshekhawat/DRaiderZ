...
/**
 * Combines PlayMontageAndWait and WaitForEvent into one task
 * Also adds ability to stop previously playing montages if desired
 * Most of the code is taken from GASDocumentation example, which in turn takes from Epic's implementation of above AbilityTasks
 */
UCLASS()
class EOD_API UTask_PlayMontageAndWaitForEvent : public UAbilityTask
{
	GENERATED_BODY()
...
public:
	...
	*/
	...
	static UTask_PlayMontageAndWaitForEvent* PlayMontageAndWaitForEvent(
			...
			bool bCancelExistingAnimMontage = true,
...
private:
	...
	UPROPERTY()
	bool bCancelExistingAnimMontage;

	/** Whether montage should be aborted if ability ends */
	UPROPERTY()
